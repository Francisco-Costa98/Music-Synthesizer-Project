...
setup	call	setup_keypad	    ; sets up keypad
	...
	movwf	PORTH, ACCESS	    ; clears PORTH
	bcf	EECON1, CFGS	    ; point to Flash program memory  
	bsf	EECON1, EEPGD	    ; access Flash program memory
	movlw	0xF0		    ; gives 2 full loops of each note for song play 
	movwf	fullsong0	    ; stores it in the count checker
	movlw	0x0A		    ; additional delay to slow song down
	movwf	delayadrs0	    ; delay register
	movlw	0xF0		    ; delay to slow song down
	movwf	fullsong1	    ; stores it in the count checker
	movlw	0x08		    ; additional delay to slow song down 
	movwf	delayadrs1	    ; song delay register
	call	song_setup0	    ; sets up first song
	call	fsrload0	    ; sets up first song
	call	song_setup1	    ; sets up second song
	call	fsrload1	    ; sets up second song
	call	song_setup2	    ; sets up third song
	call	fsrload2	    ; sets up third song
	movlw	0xF0		    ; gives 2 full loops of each note for song play 
	movwf	fullsong2	    ; stores it in the count checker
	movlw	0x0B		    ; additional delay to slow down song
	movwf	delayadrs2	    ; delay register
...
chordTable db	0xfc, 0xba, 0x7b, 0x41, 0x1d, 0x18, 0x2f, 0x58, 0x82, 0xa0, 0xaa, 0xa3, 0x92, 0x82, 0x7e, 0x87, 0x98, 0xa6, 0xa8, 0x96, 0x73, 0x47, 0x24, 0x17, 0x28, 0x56, 0x94, 0xcf, 0xf5
	constant 	song0=0x100 ; first song location in ram
	constant	song1=0x140 ; second song location in ram 
	constant	song2=0x180 ; third song location in ram 
...
int_hi	code 0x0008		    ; high vector, no low vector
	...
	tstfsz	a_test, 0	    ; checks if a is pressed on the keypad
	bra	jump		    ; branches to jump and reads note is it is pressed
	tstfsz	o_test, 0	    ; checks if 0 is pressed on the keypad
	call	songloop1	    ; calls song sub-routine if 0 is pressed
	tstfsz	o_test, 0	    ; checks if 0 is pressed on the keypad
	bra	jump		    ; branches to jump and reads note is it is pressed
	tstfsz	b_test, 0	    ; checks if b is pressed on the keypad
	...
	tstfsz	chord_test, 0	    ; checks if chord key is pressed on the keypad, if no chords are pressed no values are read
	call	read_chord	    ; plays chord if certain buttons are pressed(F,E,D,C)
...
jump	call	read		    ; reads values from table
	movff	CCPR4L, PORTH	    ; shows what frequencies are being read on PORTH
...
songloop0			    ; subroutine that delays the code from the interrupt
	dcfsnz	fullsong0			     
	call	song_delay0
	return
...
play_song0			    ; subroutine to play song
	...
	movwf	fullsong0	    ; stores it in the count checker
	movlw	0x0A
	movwf	delayadrs0	    ; delay that controls the tempo of the song played
...
read_song0			    ; read routine to read values from table
	...
	movlw	0x00		
	movwf	CCPR4H		    ; move 0 into CCPR4H to reduce the delay	
	dcfsnz	song_counter0	    ; count down to zero
	call	song_counterreset0  ; if counter is zero, the counter is reset to # of notes
	return	
...
fsrload0 
	...
	call	song_counterreset0  ; reset the address and song counter
	return
	
song_counterreset0
	movlw	.30		    ; 30 notes to read
	movwf 	song_counter0	    ; song 0 counter register is reset back to the number of notes in song
	lfsr	FSR0, song0	    ; reloads the first address back into fsr0
	return
		
; ********* PLAYS CANTINA SONG *************	
	
songloop1			    ; subroutine that delays the code from the interrupt
	dcfsnz	fullsong1			     
	call	song_delay1
	return
...
read_song1			    ; read routine to read values from table
	...
	movlw	0x00
	movwf	CCPR4H		    ; move 0 into CCPR4H to reduce the delay inbetween each sample
...
fsrload1 
	...
	call	song_counterreset1  ; if counter is zero, the counter is reset to # of notes
	return
	
song_counterreset1
	movlw	.19		    ; 19 notes to read
	movwf 	song_counter1	    ; our counter register
	lfsr	FSR1, song1	    ; reloads the first address back into fsr1
	return
	
; ********* PLAYS SWEET CHILD O'MINE *************	
	
songloop2			    ; subroutine that delays the code from the interrupt
	dcfsnz	fullsong2			     
	call	song_delay2
	return
...
fsrload2
	...
	call	song_counterreset2  ; if counter is zero, the counter is reset to # of notes
	return
	
song_counterreset2
	movlw	.31		    ; 31 notes to read
	movwf 	song_counter2	    ; our counter register
	lfsr	FSR2, song2	    ; reloads the first address back into fsr2
	return
